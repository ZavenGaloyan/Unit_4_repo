...
|  #  | Planned Action                                            | Planned Outcome                                                                                                                                           | hi | Time Estimate      | Completion Date | Criteria |
|----|-----------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------|--------------------|-----------------|----------|
...
return render_template('classes_discussion_post_comments.html', post_id=post_id, title=post_title, content=post_content, username=post_username, comments=post_comments, post_images=post_images)
 ```
 As Showing the clicked on post, its comments, and creating a new comment were all on the same page I need a way for the algorthim to go through each of the nessicary requirements such as first displaying the post along with its relative content. Then displaying all the comments in the database. Afterwards it will check whether a post comment was been requested or not. This means that once the page is renderd or a post is made the algorithm can quicky complete the nessicary tasks. This code is apart of the same function as the last piece code above described above as I needd to display a post allow user to post a comment and show comments all in the same place. One technique I used many times throughout the development was understandking what I have and using it to find what I don't. For example I used user_id from a cookie to then find the username that corresponds to the same user_id. 
...
```
The code above shows the Html for deleting posts. Instead of having a simple button that says delete I decided to use a trashcan Icon to represent the delete button improve the UI experiance by keeping it minimal. This was done by refrencing this web address "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css" 
...
|Integrate a resources page based off interests             |       ✓         |        If there was a search feature to find resources more specified          |
 ...
 (Fig.A.3 for [Feedback Meeting](#meeting3) in Appendix)
 ## Recomendation 1
 Based on the feedback provided by the client, it is clear that the platform has successfully met their expectations and requirements. However, the client mentioned some areas of improvement for further developemtn. They expressed their desire for a more specified search feature within the integrated resources page. They believe that this will help the ease of users being able to search for reasources rather then them having to loop for them. To address this, it would be recommended to enhance the resources page by implementing a search bar or some form of filtering options. This would allow users to find resources that align more closely with their specific academic needs.
 ## Peer Feedback
...
## Recomendation 2
<a id="meeting1"></a>
From the peer feedback, It was apparent that the platform meets the expectations of the users. However, they  some other areas of improvement that the client didn't highlight. As they have suggested a couple of improvements. Firstly, they mentioned the need for clearer error messages when inputting information during the account creation and login process as this would improve the overall ease of use of the program. Enhancing the error messages to provide specific details about the errors encountered would improve the user experience. Secondly, the peer mentioned the need for having a back button or cancel button while using the using the various features. Implementing such a feature would allow users to navigate more easily and correct any accidental actions without losing their progress or having to deal with the unpredictable browers back buttons.
